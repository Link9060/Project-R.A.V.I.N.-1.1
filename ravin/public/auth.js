(function () {
  ...
  function saveSession(session, user) {
    ...
    if (state.expiresAt) localStorage.setItem("ravin_token_expires_at", String(state.expiresAt)); else localStorage.removeItem("ravin_token_expires_at");
    scheduleRefresh();
  }
  ...
  function clearSession() {
    ...
    localStorage.removeItem(TOKEN_KEY); localStorage.removeItem(REFRESH_KEY); localStorage.removeItem(USER_KEY); localStorage.removeItem("ravin_token_expires_at"); localStorage.removeItem(CONVERSATION_KEY);
  }
  ...
  function createAuthUI() {
    ...
    form.addEventListener("submit", async (event) => { event.preventDefault(); error.textContent = ""; submit.disabled = true; submit.textContent = mode === "signin" ? "Signing in…" : "Creating…"; try { const data = await authRequest(mode, email.value.trim(), password.value); if (!data.session) throw new Error("Account created. Check your email to confirm it, then sign in."); saveSession(data.session, data.user); backdrop.hidden = true; updateAuthUI(); } catch (err) { error.textContent = err.message; } finally { submit.disabled = false; submit.textContent = mode === "signin" ? "Sign in" : "Create account"; } });
  ...
  function installMemoryBridge() {
    window.RavinMemory = { isConfigured: () => isSignedIn(), getStoredUrl: () => SUPABASE_FUNCTION_URL, getStoredAnonKey: () => "managed by RAVIN", saveCredentials: () => {}, async listPermanentMemories() { if (!await ensureFreshSession()) return []; const response = await fetch("/api/memories", { headers: { Authorization: `Bearer ${state.accessToken}` } }); if (!response.ok) throw new Error("Could not load memories"); const data = await response.json(); return data.permanent || []; }, async addPermanentMemory(content, category = "fact") { if (!await ensureFreshSession()) throw new Error("Please sign in again."); const response = await fetch("/api/memories", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${state.accessToken}` }, body: JSON.stringify({ content, category }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Could not save memory"); return data.memory; }, async deletePermanentMemory(id) { if (!await ensureFreshSession()) throw new Error("Please sign in again."); const response = await fetch(`/api/memories/${encodeURIComponent(id)}`, { method: "DELETE", headers: { Authorization: `Bearer ${state.accessToken}` } }); if (!response.ok) throw new Error("Could not delete memory"); } };
  }
  ...
  function installChatBridge() {
    ...
    form.addEventListener("submit", async (event) => { event.preventDefault(); event.stopImmediatePropagation(); if (!isSignedIn()) return window.RavinAuth.open(); if (!await ensureFreshSession()) return window.RavinAuth.open(); const message = input.value.trim(); if (!message) return; input.value = ""; sendBtn.disabled = true;
      ...
      try { const response = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${state.accessToken}` }, body: JSON.stringify({ message, conversation_id: localStorage.getItem(CONVERSATION_KEY) || null }) });
        if (response.status === 401 && await refreshSession()) { const retry = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${state.accessToken}` }, body: JSON.stringify({ message, conversation_id: localStorage.getItem(CONVERSATION_KEY) || null }) }); if (!retry.ok) throw new Error((await retry.json().catch(() => ({}))).error || "RAVIN could not respond"); const data = await retry.json(); if (data.conversation_id) localStorage.setItem(CONVERSATION_KEY, data.conversation_id); thinking.remove(); const reply = document.createElement("div"); reply.className = "msg ravin"; reply.innerHTML = `<div class="msg-header"><span class="msg-label">RAVIN</span></div><div class="msg-body"></div>`; reply.querySelector(".msg-body").innerHTML = window.renderMarkdown ? window.renderMarkdown(data.reply || "") : ""; chat.appendChild(reply); chat.scrollTop = chat.scrollHeight; return; }
        const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || "RAVIN could not respond"); if (data.conversation_id) localStorage.setItem(CONVERSATION_KEY, data.conversation_id); thinking.remove(); const reply = document.createElement("div"); reply.className = "msg ravin"; reply.innerHTML = `<div class="msg-header"><span class="msg-label">RAVIN</span></div><div class="msg-body"></div>`; reply.querySelector(".msg-body").innerHTML = window.renderMarkdown ? window.renderMarkdown(data.reply || "") : ""; chat.appendChild(reply); chat.scrollTop = chat.scrollHeight;
      } catch (err) { thinking.remove(); const error = document.createElement("div"); error.className = "msg error"; error.innerHTML = `<div class="msg-header"><span class="msg-label">System</span></div><div class="msg-body"></div>`; error.querySelector(".msg-body").textContent = err.message; chat.appendChild(error); } finally { sendBtn.disabled = false; input.focus(); }
    }, true);
  }
